...
var mainWindow = null;
var tray = null;
var sleepBlockerId = -1;
var pairedBluetoothDevices = loadConfig("pairedDevices", {});
var kioskMode = loadConfig("kioskMode", false);
var KIOSK_PIN = loadConfig("kioskPin", "");
...
function createWindow() {
  ...
  const icon = fs.existsSync(iconPath) ? nativeImage.createFromPath(iconPath) : void 0;
  mainWindow = new BrowserWindow({
    ...
    show: false,
    // revealed after 'ready-to-show' for clean splash
    webPreferences: {
  ...
  if (kioskMode) {
    mainWindow.webContents.on("before-input-event", (e, input) => {
      if (input.alt && input.key === "F4" || input.meta && input.key === "q" || input.control && input.key === "w") {
        e.preventDefault();
      }
    });
  }
...
var KNOWN_READERS = ["square", "stripe", "bbpos", "chipper", "reader"];
function setupBluetooth() {
  session.defaultSession.on("select-bluetooth-device", (event, deviceList, callback) => {
    ...
    if (paired) {
      callback(paired.deviceId);
      return;
    }
    const reader = deviceList.find(
      (d) => KNOWN_READERS.some((k) => (d.deviceName || "").toLowerCase().includes(k))
    );
    if (reader) {
...
function setupAutoUpdater() {
  ...
  const { autoUpdater } = require_main2();
  ...
  setInterval(() => autoUpdater.checkForUpdates().catch(console.error), 4 * 60 * 60 * 1e3);
}
